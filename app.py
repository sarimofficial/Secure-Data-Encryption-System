...
LOCKOUT_DURATION = 60  # Corrected the variable name to match usage
...
# === User login ===
    ...
    if st.button("Login"):
        if username in stored_data and stored_data[username]["Password"] == hash_password(password):
            ...
            st.error(f"Invalid Credentials! Attempts left: {remaining} attempts left.")
    ...
    if not st.session_state.authenticated_user:
        ...
        if st.button("Encrypt And Save"):
            if data and passkey:
                ...
                stored_data[st.session_state.authenticated_user]["data"][passkey] = encrypted
                save_data(stored_data)
